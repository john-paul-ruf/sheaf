 ...
 * fed row past the header is a data row. The number of data rows a table gets
...
export class RowPlan {
  ...
  readonly #declaredBySheet: ReadonlyMap<number, readonly RangeV1[]>;
  readonly #counts = new Map<string, number>();

  constructor(proposal: ProposedWorkbookV1, extents: ReadonlyMap<string, TableRowExtentV1>) {
    const declaredBySheet = new Map<number, RangeV1[]>();
    for (const table of proposal.tables) {
      if (table.source.kind === "declared-table") {
        const sheetIndex = sheetIndexOf(table.sheetKey);
        declaredBySheet.set(sheetIndex, [...(declaredBySheet.get(sheetIndex) ?? []), table.source.range]);
      }
    }
    this.#declaredBySheet = declaredBySheet;
  ...
  #columnsOf(geometry: GeometryV1, row: PlannedRowV1): [number, PlannedCellV1][] {
    const declaredRanges = geometry.declared === null && !geometry.isDelimited ? (this.#declaredBySheet.get(row.sheetIndex) ?? []) : [];
  ...
  isReadableBy(table: ProposedTableV2, row: PlannedRowV1, columnIndex: number): boolean {
    ...
    return !(this.#declaredBySheet.get(row.sheetIndex) ?? []).some((range) => inRange(range, row.rowIndex, columnIndex));
  }
