...
| `tests/e2e/fixtures/durability.ts` | Extend S02 output after receive | CP2 REMINDER-CLOCK-S03 actual-worker clock controller, identity, lifecycle and isolation |
...
| `tests/e2e/scratch-reminders.spec.ts` | Create (planned) | CP2 REMINDER-CLOCK-S03 browser self-tests, exact deadlines/restart, negative control and real-clock journey |
...
| `tests/e2e/accessibility.spec.ts` | Modify expected reminder boundary only | Dismiss/ack approved post-save reminder; preserve every original behavior, accessibility, offline, provenance, value, J1 and restart assertion |
| `tests/e2e/bundle-backup.spec.ts` | Modify expected reminder boundary only | Dismiss/ack approved post-save reminder; preserve every original behavior, accessibility, offline, provenance, value, J1 and restart assertion |
| `tests/e2e/dark-semantics.spec.ts` | Modify expected reminder boundary only | Dismiss/ack approved post-save reminder; preserve every original behavior, accessibility, offline, provenance, value, J1 and restart assertion |
| `tests/e2e/offline-guard.spec.ts` | Modify expected reminder boundary only | Dismiss/ack approved post-save reminder; preserve every original behavior, accessibility, offline, provenance, value, J1 and restart assertion |
| `tests/e2e/workbook-import.spec.ts` | Modify expected reminder boundary only | Dismiss/ack approved post-save reminder; preserve every original behavior, accessibility, offline, provenance, value, J1 and restart assertion |

| `src/application/commands/schema-commands.ts` | Modify no-op boundary and paired assertion | Existing approved no-op semantics: unchanged schema/chart request writes no authored commit or reminder; honest result mapping, stale/invalid guard preserved |
| `src/application/commands/chart-commands.ts` | Modify no-op boundary and paired assertion | Existing approved no-op semantics: unchanged schema/chart request writes no authored commit or reminder; honest result mapping, stale/invalid guard preserved |
| `src/workers/data/structure-handlers.ts` | Modify no-op boundary and paired assertion | Existing approved no-op semantics: unchanged schema/chart request writes no authored commit or reminder; honest result mapping, stale/invalid guard preserved |
| `src/application/view-models/schema.ts` | Modify no-op boundary and paired assertion | Existing approved no-op semantics: unchanged schema/chart request writes no authored commit or reminder; honest result mapping, stale/invalid guard preserved |
| `tests/unit/view-models/schema.test.ts` | Modify no-op boundary and paired assertion | Existing approved no-op semantics: unchanged schema/chart request writes no authored commit or reminder; honest result mapping, stale/invalid guard preserved |
| `tests/unit/ui/schema/structure-screen.test.tsx` | Modify no-op boundary and paired assertion | Existing approved no-op semantics: unchanged schema/chart request writes no authored commit or reminder; honest result mapping, stale/invalid guard preserved |

## Controlled lease revision r2 — 2026-09-25

Prior lease r1; checkpoint0/4. S02 fully received/released at5333da8; current implementation base823012b. Five exact compatibility specs above added together; no live owner or residual source work. These existing journeys accept scratch edits outside shared mutation helpers. CP2 adapts expected reminder dismissals at accepted-save boundaries; CP4 runs their original assertions in full e2e. No global reminder suppression, dropped assertions or production changes in this amendment. Dependencies, four checkpoint boundaries, architectural ownership and other lease paths unchanged.
...
Lease r3 includes command-layer correction for CP0 counterexamples: unchanged schema app/table/field names or required flag and identical chart definition must not commit; zero-draft schema must return an honest no-change outcome instead of throwing. Preserve validation/stale guards, existing receipt/count authority and changed-command behavior. Co-update protocol/route/VM and real worker/paired UI tests within lease. Prove no new commit/count/history/reminder mutation before and after reopen; stale requests remain refused. Existing chart null-commit mapping can carry no-change without new semantics. Do not rewrite already persisted historical commits.
...
**Commit when:** tests/e2e/scratch-reminders.spec.ts proves all authored families, dismiss→continue, interval boundaries using the actual built worker control and browser self-tests specified in REMINDER-CLOCK-S03 below, page restart persistence and home-assignment suppression. Existing CRUD/structure/chart/theme fixtures and the five r2 compatibility specs explicitly dismiss expected prompts at accepted-save boundaries while preserving their original assertions.
...
Finish responsive status layouts and stale bundle emphasis from design sources; keep safety semantics system-owned. Add negative controls for reminder deduplication and stale response routing. Update only e2e fixtures/selectors whose expected new reminders otherwise obscure the old capability assertions; the five r2 exact compatibility specs are already leased for this purpose; record any further demonstrated seam through Controlled Lease Revision.
...
## Controlled lease revision r3 — no-op producer correction

At checkpoint0/4 after ended CqY3I, source823012b unchanged; CP0 diagnostic at2a87de3 preserved. Six exact added paths cover command producers, exhaustive schema handler/VM and paired VM/structure screen regression. Existing leased protocol/routes/worker tests cover all other consumers; chart handler already maps null commit honestly. No active owner/source residual; all dependencies and four checkpoints unchanged. Same-context resume after complete receive, no source edits by Orchestrator.

## Verification

For CA-37, test first-change eligibility and every dismissal boundary at just-before/exactly-due for 10m/1h/24h/daily, repeated daily dismissal, clock rollback, restart, no-op/rejection, duplicate acknowledgements and assignment suppression. Assert encrypted scheduling survives reopen, accepted edits never wait for a prompt, and the badge persists after dismissal. CP1 policy/worker tests and CP2 real-entry assertions own these checks. J2 specification in STATE.md. Focused commands: pnpm test tests/unit/policy tests/unit/workflows tests/unit/view-models tests/unit/workers; SHEAF_PW_PORT=<port> pnpm test:e2e scratch-reminders; same for backup-status. Run full e2e once after all adapted old journeys pass. Workflow fixture negative controls remain under tests/unit/workflows/durability.test.ts; browser clock fixture self-tests belong in tests/e2e/scratch-reminders.spec.ts and must execute in the browser. Use REMINDER-CLOCK-S03 below for worker time control and its browser self-tests; node fake clocks and page-only clocks cannot prove worker timing. Retain the separate real-clock entry journey. Never sleep for hours or use test time as durable authority. Evidence test-results/f05/s03. First-journey proof J1 must be current before dispatch.
...
### REMINDER-CLOCK-S03 — F05-APPROVAL-01 / CAP-42 / CA-37 / J2

**Future ownership and prerequisite.** This is a plan-time construction assignment to S03 CP2, before its boundary proof and commit, not an existing harness or a passing result. Extend `tests/e2e/fixtures/durability.ts` after S02 is fully received and releases it; put the fixture's behavioral self-tests and journey assertions in the already-leased `tests/e2e/scratch-reminders.spec.ts`. No lease addition, new production injection API, worker replacement, protocol test opcode, build transform, or package dependency is needed. Preserve all four checkpoints and S02/J1 dependency. S03 CP1 owns policy/persisted producer; CP2 owns browser mechanism, service/route re-query, and integration; CP4 reruns J2 with old journeys.

**Source-backed mechanism.** At replan base `8c8a53d098e83a2cef04ddfca3f72db6e5884fa4`, `src/bootstrap/app-bootstrap.ts`'s `spawnDataWorker()` creates a module Worker from `../workers/data.worker.ts`; `DataWorkerClient` lazily creates it on first request. `src/workers/data.worker.ts` constructs `ClockPort` as `{ nowEpochMs: () => Date.now() }`, so changing `Date.now` inside that same live worker controls subsequent clock reads without replacing the handler, transport or database. Installed Playwright 1.63.0 exposes `page.workers()`, `page.waitForEvent('worker', ...)`, `Worker.url()` and `Worker.evaluate()`. `tests/browser/worker/runtime.ts` starts ordinary `startApp()` and supplies no clock control; the node helper's FakeClock does not establish J2. The durability fixture is absent at this base and is a planned S02 output, subsequently extended by S03.

**Entry, selection and control.** Open `/` with the existing real-entry fixture, never `/harness.html`. Register the worker listener before navigation, also inspect `page.workers()` to handle an already-started worker. Select exactly the data-worker asset URL emitted by the fresh build, matched to the bootstrap worker reference; reject zero, duplicate, blob/substitute or unexpected worker URLs rather than selecting the first worker (import and IO workers must remain untouched). Await its handle and run `worker.evaluate` before any authored mutation or dismissal under test. Startup/protection may have run already; do not assert that startup was clock-controlled. Set a finite safe-integer epoch in the worker realm by replacing only its `Date.now`, retaining its original property descriptor for restoration. Keep browser timers, performance clock, Date constructor, randomness, crypto and IndexedDB real. A proposed fixture API (new, not committed) is:

```ts
interface ReminderWorkerClock {
  setEpochMs(epochMs: number): Promise<void>;
  readEpochMs(): Promise<number>;
  restore(): Promise<void>;
}
// Inside the selected real worker, invoked by Worker.evaluate with epochMs:
Object.defineProperty(Date, "now", {
  configurable: true, writable: true, value: () => epochMs,
});
```

Each setter validates input node-side and awaits evaluation plus independent `worker.evaluate(() => Date.now())` equality before allowing the next UI action. Retain/restore the original descriptor inside that worker only; never copy a node function across realms. Reject use after worker closure or page replacement. This tests the production ClockPort's time source, not a synthetic response: all queries/dismissals still travel UI → service → real DataWorkerClient/MessagePort → real handler/policy → encrypted catalog in actual IndexedDB. CP2 must wire a normal production route re-entry/refresh through its owned routes/services to query eligibility at each boundary; a harness cannot call the policy directly or manufacture eligibility. Await the completed query/render (positive ready-state assertion) before asserting no prompt, so a pending response cannot pass an absence assertion.

**Authoritative seed and exact deadlines.** Use `protectDevice`, `importDemoWorkbook` from the existing e2e fixtures, then a genuine UI-authored edit and local acknowledgement in the scratch app. Import alone is not the trigger. Set worker epoch `T0 = 1800000000000` before this edit. Assert immediate eligibility, persistent badge and successful saved edit. Dismiss through the real UI and await its operational-write acknowledgement before advancing time. With dismissal at each exact due instant, assert these pairs through production re-query:

| Dismissal count after acknowledged dismissal | Dismissal epoch | Ineligible at | Eligible at |
|---|---:|---:|---:|
| 1 | 1800000000000 | 1800000599999 | 1800000600000 |
| 2 | 1800000600000 | 1800004199999 | 1800004200000 |
| 3 | 1800004200000 | 1800090599999 | 1800090600000 |
| 4 | 1800090600000 | 1800176999999 | 1800177000000 |
| 5 | 1800177000000 | 1800263399999 | 1800263400000 |

Intervals are 600000, 3600000, 86400000, 86400000, 86400000 milliseconds from the acknowledged dismissal, never page load or last query. Dismiss and replace the page after every row's dismissal: close the old page and await the worker close, create a new page in the same isolated context/origin, reacquire the actual newly spawned worker, reinstall the next test epoch before unlock/reopening the scratch app, then query at due-minus-one and due. If startup spawns lazily during unlock, register/evaluate the new worker before opening the app or issuing the eligibility query; wait for control acknowledgement. Assert a different worker handle, preserved triggering commit and dismissal progression, badge persistence, and no authored count increment from dismissal/re-query. Read stored state via the production reopen/query path; no direct catalog seed, timestamp rewrite or in-memory state transplant. Later edits, rejected/no-op edits, stale dismissal identity, clock rollback and home-assignment suppression retain CP1/CP2 assertions. Clock rollback must not erase escalation or advance eligibility. Home assignment uses S02's real path.

**Exact browser self-tests and integration selectors (all new, CP2-owned).** In `tests/e2e/scratch-reminders.spec.ts` register these literal test titles:

- `J2 clock bridge controls only the built data worker`: independent readback equals T0 and then T0+1; page time remains real; invalid epoch and closed-worker operations reject; restoration restores the original worker Date.now descriptor and real time; new page/worker does not inherit the old controller.
- `J2 page-only clock cannot advance worker eligibility`: dismiss at T0, leave worker fixed at T0, change only page `Date.now` with `page.evaluate` to T0+600000, re-enter/query through production UI and assert completed ineligible response/no prompt and unchanged badge/count. Independently assert worker still reads T0. Restore page descriptor, advance worker to T0+600000, re-query and require eligibility. This negative control fails if the consumer trusts renderer time. Do not use context-wide clock facilities whose worker reach is ambiguous.
- `J2 exact dismissal deadlines survive page replacement and repeat daily`: execute the full table with fresh worker/reopen on every dismissal and assertions above; encrypted state, not the harness controller, carries progression.
- `J2 real clock authored edit dismiss reopen`: separate fresh context with no clock overrides; real entry → authored edit → immediate prompt → dismissal → continued edit → page replacement/unlock → badge and saved edits preserved, prompt deferred. Do not wait ten minutes; this case proves real-clock composition, while the table proves exact boundaries.

Run discovery first: `SHEAF_PW_PORT=<assigned-port> pnpm test:e2e scratch-reminders --list`. Require all four exact titles discovered once. Run the fixture self-tests with `SHEAF_PW_PORT=<assigned-port> pnpm test:e2e scratch-reminders --grep 'J2 (clock bridge|page-only clock)' --workers=1 --timeout=240000 --global-timeout=900000`; require two tests, no skips. Run the complete J2 spec with `SHEAF_PW_PORT=<assigned-port> pnpm test:e2e scratch-reminders --workers=1 --timeout=240000 --global-timeout=900000`; require all four plus authored-family assertions, no skips. The e2e project discovers `tests/e2e/**/*.spec.ts`; package script is `playwright test --project=e2e`. Test titles/API above are planned outputs; installed runner facilities are inspected facts. A failing self-test blocks the CP2 proof; a passing unit workflow test does not replace it.

**Isolation, freshness, teardown and evidence.** Each test uses a fresh Playwright BrowserContext with real IndexedDB `sheaf-local`; only page replacement within that test retains storage. Keep S02's accepted setup/teardown conventions and real internal services. No external provider is needed. Acquire `dist:build` and `playwright:output`, and the assigned port before invocation. Existing Playwright webServer runs `pnpm build && pnpm preview --port <port> --strictPort` with `reuseExistingServer: false`. Record source HEAD plus dirty source inventory/digests, effective config/package/lockfile digests, actual served `window.__sheafBuildId`, and SHA-256 of emitted index/main/data-worker assets; build ID alone does not identify uncommitted checkpoint work. Match worker URL to the recorded emitted asset and served bytes, record that identity before control, and reject any stale or unrelated server. Do not start or reuse S02's live server.

Use test-local `try/finally` to restore descriptors on live realms, close pages and await worker closure with a 10-second bound, then close the context even on failures. Closed realms need no restore; suppress only the expected closed-realm restoration case, not assertion errors. Never accept blocked database deletion as success; context disposal provides final isolation. Bound worker acquisition/evaluation/ack waits at 10 seconds (surface timeout as failure, then dispose context); preserve existing 90-second crypto/UI allowances inside the 240-second test bound. Stop only the server/process owned by this invocation. Attach build/worker identity, epoch/deadline table, acknowledgement and reopen evidence through `testInfo.outputPath`/attachments in Playwright's per-test output, then collate under `test-results/f05/s03`; this output is ignored evidence under the resource lock, not a new git-add lease. Never record secrets or decrypted user data.

**Resumption recheck and receipt.** After S02 r7 is fully received, Orchestrator and S03 CP0 read actual landed bootstrap/client/worker/protocol, fixture, J1 evidence, and effective runner configuration. Reconfirm dynamic worker Date.now lookup, asset URL/selection, dismissal/query response names and normal route re-query; adapt this plan to actual landed contracts before dispatch, not to guessed frozen S02 signatures. If the time source changes, amend the concrete mechanism/ownership through Controlled Lease Revision before dependent proof. This is a compatibility recheck of a specified solution, not an investigation replacing the known assignment. No simultaneous fixture ownership; S02 instructions/lease are unchanged. On receive, Orchestrator reconciles STATE VB-02/J2, CAP-42, CA-37 and F05-APPROVAL-01 with this specification; policy remains agreed, producer/proof remain planned until S03's actual evidence. First narrow journey stays S02/J1; downstream J2 consumers cannot claim verified reminders from this plan-only correction.
