...
createEventStore writes one segment per commit and keeps a growing tail. chainState currently requires the last local commit to be in that tail. S02 owns the current-producer exporter/cursor; no compactor or nonempty audit replay is accepted. Its r4 candidate is uncommitted at replan HEAD. This is one coherent storage/restart capability, split from first bundle because its projection/history working set and proof matrix are substantially larger.
