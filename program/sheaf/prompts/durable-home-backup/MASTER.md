...
S04/S05/S06 source leases are disjoint but full-session dist:build/playwright:output reservations must serialize under current envelopes. Provider input blockage need not stop S06. A distinct port does not isolate shared outputs. After receiving this plan correction, Orchestrator refreshes its ledger from STATE and performs scoped planning-completeness recheck before affected dispatch. DEC-71/72, design fill, provider qualification and demo verdict remain separate gates.
