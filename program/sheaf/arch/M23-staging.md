...
**M23 — Staging (`src/import/staging/`)**
- `stage.ts`: `IMPORT_STAGE_VERSION = 2`; `ImportStageV1` gains `format: ImportFormatV1` (`"delimited" | WorkbookFormatV1`, `IMPORT_FORMATS`), `destination: ImportDestinationV1` (`new-app | existing-app{appId bytes}` — existing-app only for delimited), `selectedSheets: number[]` (ascending, unique, non-empty; delimited = `[0]`), `preflight: PreflightReportV1 | null` + `inventory: StagedSheetSummaryV1[] | null` (exactly one set), `proposal: ProposedWorkbookV1 | null`, `reviewEdits: WorkbookReviewEditV1[]`.
- `lifecycle.ts`: `stageWithProposal`/`stageWithReviewEdit` take S02's workbook shapes (`applyWorkbookReviewEdit`). New `readStageRows(ports, root, workflow)` — the rows a stage names even when its payload no longer decodes (retained ∪ temporary ids): an F02 (v1) stage is **stale**, abandoned and collected with a receipt by the unlock sweep (`cleanup.ts` uses it), never an integrity error.
- NEW `fact-codec.ts` (CR7): `encodeFactStreamItem`, `decodeFactStreamItem` — exact-key inverse of M65's canonical mapping (CA-17 stage round trip).
- NEW `workbook-proposal-codec.ts` (CR7): exact-key codec for `ProposedWorkbookV1`, `WorkbookStatementV1`, `WorkbookEvidenceV1`, `WorkbookReviewEditV1` (reuses `proposal-codec.ts` primitives and F02 mappings). Rule values refuse enum/reference.
- NEW `row-plan.ts` (CR7): `RowPlan(proposal, extents)` + `walkRows(items, visitor)` — places each staged row as inference did, **in stream order** (a declared table takes rows only after its fact; "loose" = outside the declared tables seen so far); `assertMatches(proposal)` refuses a plan whose data-row counts differ from the reviewed exact counts; `sourceTextAt`.
- NEW `import-commit.ts` (CR7): `sealImportCommit(...)` — the one import-class commit builder (M09 `sealEventCommit`, one segment per commit, accumulated-set chain check); `ImportChainV1`, `EMPTY_IMPORT_CHAIN`. Promotion and append both use it.
- `promotion.ts` rewritten for workbooks: `allocateSchema(entropy, proposal, {firstTableOrdinal?, schemaRevision?})` (selected sheets only; joined tables promote into their head; applied relationships; record rules as warning-severity `validation.record-rule`); `buildRecords` (two passes: keyed tables' ids + parent key-text maps, then records; references `reference{recordId}` or `invalid-preserved{key}`; optional external `referenceExists`); `writeSnapshots`, `inertItemsOf`, `decisionsOf`, `tableEvents`, `decisionEvent`, `rootSealer`, `sealRecordPages` exported; `INERT_REASON_OF` (M65 → M01, total, test-pinned). `PROMOTION_REJECTIONS` += **`append-too-large`**. Checkpoint written with every F03 root (relationships, validationRules, inertItems, inferenceDecisions, importLineages; sheets with classification + snapshotRevision); head lists every snapshot manifest. Memory bound: one RecordId per keyed-table row + one entry per distinct parent key text, ≤ the D31 250,000-cell budget.
- NEW `append.ts`: `appendTable(deps, {loaded, facts, target, deviceId})` (D38, CA-23): one import commit (`table.created{table, sourceSheet}`, `field.created`, `enum.changed`, `record.created`×rows, `inference-decision.recorded`; no `import.accepted`); caps `APPEND_MAX_EVENTS = 10_000`, `APPEND_MAX_SEGMENT_BYTES = 16_777_216` (padded estimate from uncompressed bytes) → `append-too-large`; source and snapshot re-sealed under the app key; head grows `sourceManifests`/`snapshotManifests`/`eventSegments`, `schemaRevision + 1`; one transaction deleting the superseded head and the workflow. `record.created` payload encoder injected (M33 owns it).
