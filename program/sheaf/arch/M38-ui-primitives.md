...
and theme). Reconciled against the tree at `5ab3b07` (F02 final).
...
## Backlog carried from F02 (owners: the next session holding this lease)

F02's UI sessions did **not** hold `src/ui/primitives/`, so three controls were
composed inside their feature modules instead and belong here when a lease
allows it:

- **CTL-044** (radio choice with a disabled state) — composed in M43 from React
  Aria's `RadioGroup`/`Radio` for SCR-017's destination choice, meeting the
  control contract locally (visible label, reason in text, 44px).
- **`TextArea`** — the review screen's enum-options editor, same situation.
- **`InlineLink`'s `externalHandoff` still throws.** F02 gave the product its
  first real external handoffs (`tel:`, `mailto:`, maps), and M44 builds those
  hrefs itself; the primitive's approved variant is still unwritten. Whoever
  writes it must keep invariant 10's "no unapproved handoff" property that the
  throw is standing in for today.
...
- 2026-09-08 — reconciled by Roshi (F01 final pass): head export list corrected
  to include the four F-03 primitives (the seeded list predated the replan);
  staple merged.
- 2026-09-08 — F02: consumed unchanged again by SESSION-07/08 (`8a665c1`,
